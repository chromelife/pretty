...
class cimages extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('images', 'images', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// created_at
		$this->created_at = new cField('images', 'images', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('images', 'images', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->updated_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['updated_at'] = &$this->updated_at;

		// image_url
		$this->image_url = new cField('images', 'images', 'x_image_url', 'image_url', '`image_url`', '`image_url`', 200, -1, TRUE, '`image_url`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['image_url'] = &$this->image_url;

		// title
		$this->title = new cField('images', 'images', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title'] = &$this->title;

		// isVisible
		$this->isVisible = new cField('images', 'images', 'x_isVisible', 'isVisible', '`isVisible`', '`isVisible`', 16, -1, FALSE, '`isVisible`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->isVisible->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->image_url->Exportable) $Doc->ExportCaption($this->image_url);
				if ($this->title->Exportable) $Doc->ExportCaption($this->title);
				if ($this->isVisible->Exportable) $Doc->ExportCaption($this->isVisible);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->image_url->Exportable) $Doc->ExportCaption($this->image_url);
				if ($this->title->Exportable) $Doc->ExportCaption($this->title);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->image_url->Exportable) $Doc->ExportField($this->image_url);
					if ($this->title->Exportable) $Doc->ExportField($this->title);
					if ($this->isVisible->Exportable) $Doc->ExportField($this->isVisible);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->image_url->Exportable) $Doc->ExportField($this->image_url);
					if ($this->title->Exportable) $Doc->ExportField($this->title);
