...
class cimages_edit extends cimages {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->image_url);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// image_url
			if (!($this->image_url->ReadOnly) && !$this->image_url->Upload->KeepFile) {
				$this->image_url->Upload->DbValue = $rs->fields('image_url'); // Get original value
...
?>
<form name="fimagesedit" id="fimagesedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if (@$_POST["fa_x_image_url"] == "0") { ?>
<input type="hidden" name="fa_x_image_url" id= "fa_x_image_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_image_url" id= "fa_x_image_url" value="1">
<?php } ?>
