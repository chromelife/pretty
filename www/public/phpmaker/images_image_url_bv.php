...
$images_image_url_blobview = NULL; // Initialize page object first

class cimages_image_url_blobview extends cimages {
	...
	// Page object name
	var $PageObjName = 'images_image_url_blobview';
	...
	function Page_Main() {
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('image_url');
				...
				if (trim(strval($this->Recordset->fields('image_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('image_url') . "\"");
				}
...
// Create page object
if (!isset($images_image_url_blobview)) $images_image_url_blobview = new cimages_image_url_blobview();

// Page init
$images_image_url_blobview->Page_Init();

// Page main
$images_image_url_blobview->Page_Main();
?>
<?php
$images_image_url_blobview->Page_Terminate();
?>
