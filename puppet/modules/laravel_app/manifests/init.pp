class laravel_app
{
	...
	exec { 'clean www directory':
		command => "/bin/sh -c 'cd /vagrant/www && find -mindepth 1 -delete'",
		unless => [ "test -f /var/www/composer.json", "test -d /vagrant/www/app" ],
	...
	exec { 'create laravel project':
		...
		creates => "/vagrant/www/composer.json",
		timeout => 1800,
		logoutput => true
	}

	exec { 'update packages':
        command => "/bin/sh -c 'cd /vagrant/www/ && COMPOSER_HOME=\'/vagrant/www\' composer --verbose --prefer-dist update'",
        require => [Package['git-core'], Package['php5'], Exec['global composer']],
        onlyif => [ "test -f /vagrant/www/composer.json", "test -d /vagrant/www/vendor" ],
        timeout => 900,
        logoutput => true
	}
	...
	exec { 'install packages':
        ...
        onlyif => [ "test -f /vagrant/www/composer.json" ],
